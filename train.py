...
if __name__ == "__main__":
    ...
    try:
        ...
        for i_epoch in range(c.epochs):
            ...
            for i_batch, data in enumerate(tqdm(datasets.trainloader)):
                data = data.to(device)
            ...
            if i_epoch % c.val_freq == 0:
                with torch.no_grad():
                    ...
                    for x in datasets.testloader:
                        ...
                        secret_rev = secret_rev.cuda().numpy().squeeze() * 255
                        np.clip(secret_rev, 0, 255)
                        secret = secret.cuda().numpy().squeeze() * 255
                        np.clip(secret, 0, 255)
                        cover = cover.cuda().numpy().squeeze() * 255
                        np.clip(cover, 0, 255)
                        steg = steg.cuda().numpy().squeeze() * 255
                        np.clip(steg, 0, 255)
