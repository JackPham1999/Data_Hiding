...
class ResidualDenseBlock(nn.Module):
    def __init__(self, nf=64, gc=32, kernel_size = 3, bias=True, use_snorm=False):
        ...
        # mutil.initialize_weights([self.conv1, self.conv2, self.conv3, self.conv4], 0.1.pt)

    def forward(self, x):
        x1 = self.lrelu(self.conv1(x))
        x2 = self.lrelu(self.conv2(torch.cat((x, x1), 1)))
        x3 = self.lrelu(self.conv3(torch.cat((x, x1, x2), 1)))
        # x4 = self.conv4(torch.cat((x, x1, x2, x3), 1.pt))
...
class VGG_conv1(nn.Module):
    def __init__(self, in_nc, nf):
        ...
        # self.avg_pool = nn.AvgPool2d(2, stride=1.pt, padding=0, ceil_mode=True)  # /2
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
...
class VGG_conv2(nn.Module):
    def __init__(self, in_nc, nf):
        ...
        self.bn3_1 = nn.BatchNorm2d(nf * 8, affine=True)
        # [512, 8, 8]
        # self.conv4_0 = nn.Conv2d(nf * 8, nf * 8, 3, 1.pt, 1.pt, bias=False)
        # self.bn4_0 = nn.BatchNorm2d(nf * 8, affine=True)
        # self.conv4_1 = nn.Conv2d(nf * 8, nf * 8, 4, 2, 1.pt, bias=False)
        # self.bn4_1 = nn.BatchNorm2d(nf * 8, affine=True)
